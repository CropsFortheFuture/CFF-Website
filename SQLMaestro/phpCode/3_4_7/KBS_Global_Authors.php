 ...
 */
    ...
    class KBS_Global_AuthorsPage extends Page
    {
        protected function DoBeforeCreate()
        {
            ...
            $field = new StringField('Database Authors');
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('KBS_Global_Authorsssearch', $this->dataset,
                array('ID', 'Database Authors', 'email'),
                array($this->RenderText('ID'), $this->RenderText('Database Authors'), $this->RenderText('Email')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('KBS_Global_Authorsasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('ID', $this->RenderText('ID')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('Database Authors', $this->RenderText('Database Authors')));
        ...
        protected function AddFieldColumns(Grid $grid)
        {
            ...
            //
            // View column for Database Authors field
            //
            $column = new TextViewColumn('Database Authors', 'Database Authors', $this->dataset);
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_Global_AuthorsGrid_Database Authors_handler_list');
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_Global_AuthorsGrid_email_handler_list');
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
        }
        ...
        protected function AddSingleRecordViewColumns(Grid $grid)
        {
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_Global_AuthorsGrid_Database Authors_handler_view');
            $grid->AddSingleRecordViewColumn($column);
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_Global_AuthorsGrid_email_handler_view');
            $grid->AddSingleRecordViewColumn($column);
        }
        ...
        {
            //
            // Edit column for Database Authors field
            //
            $editor = new TextEdit('database_authors_edit');
            $editor->SetSize(100);
            $editor->SetMaxLength(100);
            $editColumn = new CustomEditColumn('Database Authors', 'Database Authors', $editor, $this->dataset);
        ...
        public function GetModalGridDeleteHandler() { return 'KBS_Global_Authors_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'KBS_Global_AuthorsGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
            ...
            $column = new TextViewColumn('Database Authors', 'Database Authors', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Global_AuthorsGrid_Database Authors_handler_list', $column);
            ...
            $column = new TextViewColumn('email', 'Email', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Global_AuthorsGrid_email_handler_list', $column);
            GetApplication()->RegisterHTTPHandler($handler);//
            // View column for Database Authors field
            //
            $column = new TextViewColumn('Database Authors', 'Database Authors', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Global_AuthorsGrid_Database Authors_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            //
            // View column for email field
            //
            $column = new TextViewColumn('email', 'Email', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Global_AuthorsGrid_email_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            return $result;
        }
    ...
    try
    {
        $Page = new KBS_Global_AuthorsPage("KBS_Global_Authors.php", "KBS_Global_Authors", GetCurrentUserGrantForDataSource("KBS_Global_Authors"), 'UTF-8');
        $Page->SetShortCaption('KBS Global Authors');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('KBS Global Authors');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("KBS_Global_Authors"));
