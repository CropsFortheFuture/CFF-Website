 ...
 */
    ...
    class KBS_Biomass_UsesPage extends Page
    {
        protected function DoBeforeCreate()
        {
            ...
            $field = new IntegerField('biomass_energy');
            $this->dataset->AddField($field, false);
            $field = new IntegerField('biomass_heat');
            $this->dataset->AddField($field, false);
            $field = new IntegerField('biomass_reagent');
            $this->dataset->AddField($field, false);
            $field = new StringField('kbs_biomuse_others');
            $this->dataset->AddField($field, false);
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('KBS_Biomass_Usesssearch', $this->dataset,
                array('cropID_name_var_lndrce', 'biomass_energy', 'biomass_heat', 'biomass_reagent', 'kbs_biomuse_others'),
                array($this->RenderText('CropID'), $this->RenderText('Biomass Energy'), $this->RenderText('Biomass Heat'), $this->RenderText('Biomass Reagent'), $this->RenderText('Kbs Biomuse Others')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('KBS_Biomass_Usesasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            ...
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('biomass_energy', $this->RenderText('Biomass Energy')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('biomass_heat', $this->RenderText('Biomass Heat')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('biomass_reagent', $this->RenderText('Biomass Reagent')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('kbs_biomuse_others', $this->RenderText('Kbs Biomuse Others')));
        }
        ...
        protected function AddFieldColumns(Grid $grid)
        {
            ...
            //
            // View column for biomass_energy field
            //
            $column = new TextViewColumn('biomass_energy', 'Biomass Energy', $this->dataset);
            $column->SetOrderable(true);
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
            ...
            //
            // View column for biomass_heat field
            //
            $column = new TextViewColumn('biomass_heat', 'Biomass Heat', $this->dataset);
            $column->SetOrderable(true);
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
            ...
            //
            // View column for biomass_reagent field
            //
            $column = new TextViewColumn('biomass_reagent', 'Biomass Reagent', $this->dataset);
            $column->SetOrderable(true);
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
            ...
            //
            // View column for kbs_biomuse_others field
            //
            $column = new TextViewColumn('kbs_biomuse_others', 'Kbs Biomuse Others', $this->dataset);
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_Biomass_UsesGrid_kbs_biomuse_others_handler_list');
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
        }
        ...
        protected function AddSingleRecordViewColumns(Grid $grid)
        {
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_Biomass_UsesGrid_kbs_biomuse_others_handler_view');
            $grid->AddSingleRecordViewColumn($column);
        }
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            //
            // Edit column for biomass_energy field
            //
            $editor = new TextEdit('biomass_energy_edit');
            $editColumn = new CustomEditColumn('Biomass Energy', 'biomass_energy', $editor, $this->dataset);
            $editColumn->SetAllowSetToNull(true);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
            
            //
            // Edit column for biomass_heat field
            //
            $editor = new TextEdit('biomass_heat_edit');
            $editColumn = new CustomEditColumn('Biomass Heat', 'biomass_heat', $editor, $this->dataset);
            $editColumn->SetAllowSetToNull(true);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
            
            //
            // Edit column for biomass_reagent field
            //
            $editor = new TextEdit('biomass_reagent_edit');
            $editColumn = new CustomEditColumn('Biomass Reagent', 'biomass_reagent', $editor, $this->dataset);
            $editColumn->SetAllowSetToNull(true);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
            
            //
            // Edit column for kbs_biomuse_others field
            //
            $editor = new TextAreaEdit('kbs_biomuse_others_edit', 50, 8);
            $editColumn = new CustomEditColumn('Kbs Biomuse Others', 'kbs_biomuse_others', $editor, $this->dataset);
            $editColumn->SetAllowSetToNull(true);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
        }
        ...
        public function GetModalGridDeleteHandler() { return 'KBS_Biomass_Uses_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'KBS_Biomass_UsesGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
            ...
            $column = new TextViewColumn('kbs_biomuse_others', 'Kbs Biomuse Others', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Biomass_UsesGrid_kbs_biomuse_others_handler_list', $column);
            GetApplication()->RegisterHTTPHandler($handler);//
            // View column for kbs_biomuse_others field
            //
            $column = new TextViewColumn('kbs_biomuse_others', 'Kbs Biomuse Others', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Biomass_UsesGrid_kbs_biomuse_others_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            return $result;
        }
    ...
    try
    {
        $Page = new KBS_Biomass_UsesPage("KBS_Biomass_Uses.php", "KBS_Biomass_Uses", GetCurrentUserGrantForDataSource("KBS_Biomass_Uses"), 'UTF-8');
        $Page->SetShortCaption('KBS Biomass Uses');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('KBS Biomass Uses');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("KBS_Biomass_Uses"));
