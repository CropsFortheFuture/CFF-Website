 ...
 */
    ...
    class KBS_Crop_Where_ResearchedPage extends Page
    {
        protected function DoBeforeCreate()
        {
            $this->dataset = new TableDataset(
                ...
                '`KBS_Crop_Where_Researched`');
            $field = new IntegerField('where_researched_id', null, null, true);
            ...
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, false);
            $field = new StringField('where_researched');
            $this->dataset->AddField($field, false);
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('KBS_Crop_Where_Researchedssearch', $this->dataset,
                array('where_researched_id', 'institute_id', 'cropID_name_var_lndrce', 'where_researched', 'metadata_id'),
                array($this->RenderText('Where Researched Id'), $this->RenderText('Institute Id'), $this->RenderText('CropID'), $this->RenderText('Where Researched'), $this->RenderText('Metadata Id')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('KBS_Crop_Where_Researchedasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('where_researched_id', $this->RenderText('Where Researched Id')));
            ...
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('where_researched', $this->RenderText('Where Researched')));
        ...
        {
            //
            // View column for where_researched_id field
            //
            $column = new TextViewColumn('where_researched_id', 'Where Researched Id', $this->dataset);
            ...
            //
            // View column for where_researched field
            //
            $column = new TextViewColumn('where_researched', 'Where Researched', $this->dataset);
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_Crop_Where_ResearchedGrid_where_researched_handler_list');
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
        ...
        protected function AddSingleRecordViewColumns(Grid $grid)
        {
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_Crop_Where_ResearchedGrid_where_researched_handler_view');
            $grid->AddSingleRecordViewColumn($column);
        ...
        {
            //
            // Edit column for where_researched_id field
            //
            $editor = new TextEdit('where_researched_id_edit');
            $editColumn = new CustomEditColumn('Where Researched Id', 'where_researched_id', $editor, $this->dataset);
            ...
            //
            // Edit column for where_researched field
            //
            $editor = new TextEdit('where_researched_edit');
            $editor->SetSize(100);
            $editor->SetMaxLength(100);
            $editColumn = new CustomEditColumn('Where Researched', 'where_researched', $editor, $this->dataset);
            $editColumn->SetAllowSetToNull(true);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
        ...
        public function GetModalGridDeleteHandler() { return 'KBS_Crop_Where_Researched_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'KBS_Crop_Where_ResearchedGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
            ...
            $column = new TextViewColumn('where_researched', 'Where Researched', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Crop_Where_ResearchedGrid_where_researched_handler_list', $column);
            GetApplication()->RegisterHTTPHandler($handler);//
            // View column for where_researched field
            //
            $column = new TextViewColumn('where_researched', 'Where Researched', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Crop_Where_ResearchedGrid_where_researched_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            return $result;
        }
    ...
    try
    {
        $Page = new KBS_Crop_Where_ResearchedPage("KBS_Crop_Where_Researched.php", "KBS_Crop_Where_Researched", GetCurrentUserGrantForDataSource("KBS_Crop_Where_Researched"), 'UTF-8');
        $Page->SetShortCaption('KBS Crop Where Researched');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('KBS Crop Where Researched');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("KBS_Crop_Where_Researched"));
