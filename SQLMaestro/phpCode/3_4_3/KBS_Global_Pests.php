 ...
 */
    ...
    class KBS_Global_PestsPage extends Page
    {
        protected function DoBeforeCreate()
        {
            $this->dataset = new TableDataset(
                ...
                '`KBS_Global_Pests`');
            $field = new IntegerField('pest_id', null, null, true);
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, true);
            $field = new StringField('pest_sci_name');
            $this->dataset->AddField($field, false);
            $field = new StringField('pest_name');
            $this->dataset->AddField($field, false);
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('KBS_Global_Pestsssearch', $this->dataset,
                array('pest_id', 'pest_sci_name', 'pest_name', 'metadata_id'),
                array($this->RenderText('Pest Id'), $this->RenderText('Pest Sci Name'), $this->RenderText('Pest Name'), $this->RenderText('Metadata Id')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('KBS_Global_Pestsasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('pest_id', $this->RenderText('Pest Id')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('pest_sci_name', $this->RenderText('Pest Sci Name')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('pest_name', $this->RenderText('Pest Name')));
        ...
        {
            //
            // View column for pest_id field
            //
            $column = new TextViewColumn('pest_id', 'Pest Id', $this->dataset);
            ...
            //
            // View column for pest_sci_name field
            //
            $column = new TextViewColumn('pest_sci_name', 'Pest Sci Name', $this->dataset);
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_Global_PestsGrid_pest_sci_name_handler_list');
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
            
            //
            // View column for pest_name field
            //
            $column = new TextViewColumn('pest_name', 'Pest Name', $this->dataset);
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_Global_PestsGrid_pest_name_handler_list');
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
        ...
        protected function AddSingleRecordViewColumns(Grid $grid)
        {
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_Global_PestsGrid_pest_sci_name_handler_view');
            $grid->AddSingleRecordViewColumn($column);
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_Global_PestsGrid_pest_name_handler_view');
            $grid->AddSingleRecordViewColumn($column);
        ...
        {
            //
            // Edit column for pest_id field
            //
            $editor = new TextEdit('pest_id_edit');
            $editColumn = new CustomEditColumn('Pest Id', 'pest_id', $editor, $this->dataset);
            ...
            //
            // Edit column for pest_sci_name field
            //
            $editor = new TextEdit('pest_sci_name_edit');
            $editor->SetSize(100);
            $editor->SetMaxLength(100);
            $editColumn = new CustomEditColumn('Pest Sci Name', 'pest_sci_name', $editor, $this->dataset);
            $editColumn->SetAllowSetToNull(true);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
            
            //
            // Edit column for pest_name field
            //
            $editor = new TextEdit('pest_name_edit');
            $editor->SetSize(100);
            $editor->SetMaxLength(100);
            $editColumn = new CustomEditColumn('Pest Name', 'pest_name', $editor, $this->dataset);
            $editColumn->SetAllowSetToNull(true);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
        ...
        public function GetModalGridDeleteHandler() { return 'KBS_Global_Pests_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'KBS_Global_PestsGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
            ...
            $column = new TextViewColumn('pest_sci_name', 'Pest Sci Name', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Global_PestsGrid_pest_sci_name_handler_list', $column);
            ...
            $column = new TextViewColumn('pest_name', 'Pest Name', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Global_PestsGrid_pest_name_handler_list', $column);
            GetApplication()->RegisterHTTPHandler($handler);//
            // View column for pest_sci_name field
            //
            $column = new TextViewColumn('pest_sci_name', 'Pest Sci Name', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Global_PestsGrid_pest_sci_name_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            //
            // View column for pest_name field
            //
            $column = new TextViewColumn('pest_name', 'Pest Name', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Global_PestsGrid_pest_name_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            return $result;
        }
    ...
    try
    {
        $Page = new KBS_Global_PestsPage("KBS_Global_Pests.php", "KBS_Global_Pests", GetCurrentUserGrantForDataSource("KBS_Global_Pests"), 'UTF-8');
        $Page->SetShortCaption('KBS Global Pests');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('KBS Global Pests');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("KBS_Global_Pests"));
