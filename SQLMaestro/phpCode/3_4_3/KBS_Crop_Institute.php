 ...
 */
    ...
    class KBS_Crop_InstitutePage extends Page
    {
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('KBS_Crop_Institutessearch', $this->dataset,
                array('institute_id', 'cropID_name_var_lndrce', 'institute_name', 'info_id'),
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('KBS_Crop_Instituteasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
        ...
        protected function AddFieldColumns(Grid $grid)
        {
            ...
            $column->SetFullTextWindowHandlerName('KBS_Crop_InstituteGrid_institute_name_handler_list');
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
        ...
        protected function AddSingleRecordViewColumns(Grid $grid)
        {
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_Crop_InstituteGrid_institute_name_handler_view');
            $grid->AddSingleRecordViewColumn($column);
        ...
        public function GetModalGridDeleteHandler() { return 'KBS_Crop_Institute_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'KBS_Crop_InstituteGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
            ...
            $column = new TextViewColumn('institute_name', 'Institute Name', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Crop_InstituteGrid_institute_name_handler_list', $column);
            GetApplication()->RegisterHTTPHandler($handler);//
            // View column for institute_name field
            //
            $column = new TextViewColumn('institute_name', 'Institute Name', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Crop_InstituteGrid_institute_name_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            return $result;
        }
    ...
    try
    {
        $Page = new KBS_Crop_InstitutePage("KBS_Crop_Institute.php", "KBS_Crop_Institute", GetCurrentUserGrantForDataSource("KBS_Crop_Institute"), 'UTF-8');
        $Page->SetShortCaption('KBS Crop Institute');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('KBS Crop Institute');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("KBS_Crop_Institute"));
