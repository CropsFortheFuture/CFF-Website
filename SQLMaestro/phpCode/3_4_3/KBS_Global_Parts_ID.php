 ...
 */
    ...
    class KBS_Global_Parts_IDPage extends Page
    {
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('KBS_Global_Parts_IDssearch', $this->dataset,
                array('part_id', 'part_name'),
                array($this->RenderText('Part Id'), $this->RenderText('Part Name')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('KBS_Global_Parts_IDasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('part_id', $this->RenderText('Part Id')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('part_name', $this->RenderText('Part Name')));
        }
        ...
        {
            //
            // View column for part_id field
            //
            $column = new TextViewColumn('part_id', 'Part Id', $this->dataset);
            ...
            //
            $column = new TextViewColumn('part_name', 'Part Name', $this->dataset);
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_Global_Parts_IDGrid_part_name_handler_list');
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
        }
        ...
        protected function AddSingleRecordViewColumns(Grid $grid)
        {
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_Global_Parts_IDGrid_part_name_handler_view');
            $grid->AddSingleRecordViewColumn($column);
        }
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            //
            $editor = new TextEdit('part_id_edit');
            $editColumn = new CustomEditColumn('Part Id', 'part_id', $editor, $this->dataset);
        ...
        public function GetModalGridDeleteHandler() { return 'KBS_Global_Parts_ID_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'KBS_Global_Parts_IDGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
            ...
            $column = new TextViewColumn('part_name', 'Part Name', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Global_Parts_IDGrid_part_name_handler_list', $column);
            GetApplication()->RegisterHTTPHandler($handler);//
            // View column for part_name field
            //
            $column = new TextViewColumn('part_name', 'Part Name', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Global_Parts_IDGrid_part_name_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            return $result;
        }
    ...
    try
    {
        $Page = new KBS_Global_Parts_IDPage("KBS_Global_Parts_ID.php", "KBS_Global_Parts_ID", GetCurrentUserGrantForDataSource("KBS_Global_Parts_ID"), 'UTF-8');
        $Page->SetShortCaption('KBS Global Parts ID');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('KBS Global Parts ID');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("KBS_Global_Parts_ID"));
