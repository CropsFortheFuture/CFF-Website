 ...
 */
    ...
    class KBS_Parameter_ReferencesPage extends Page
    {
        protected function DoBeforeCreate()
        {
            $this->dataset = new TableDataset(
                ...
                '`KBS_Parameter_References`');
            $field = new IntegerField('ref_id', null, null, true);
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('KBS_Parameter_Referencesssearch', $this->dataset,
                array('ref_id', 'cropID_name_var_lndrce', 'reference', 'info_id'),
                array($this->RenderText('Ref Id'), $this->RenderText('CropID'), $this->RenderText('Reference'), $this->RenderText('Info Id')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('KBS_Parameter_Referencesasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('ref_id', $this->RenderText('Ref Id')));
        ...
        {
            //
            // View column for ref_id field
            //
            $column = new TextViewColumn('ref_id', 'Ref Id', $this->dataset);
            ...
            $column->SetFullTextWindowHandlerName('KBS_Parameter_ReferencesGrid_reference_handler_list');
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
        ...
        protected function AddSingleRecordViewColumns(Grid $grid)
        {
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_Parameter_ReferencesGrid_reference_handler_view');
            $grid->AddSingleRecordViewColumn($column);
        ...
        public function GetModalGridDeleteHandler() { return 'KBS_Parameter_References_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'KBS_Parameter_ReferencesGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
            ...
            $column = new TextViewColumn('reference', 'Reference', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Parameter_ReferencesGrid_reference_handler_list', $column);
            GetApplication()->RegisterHTTPHandler($handler);//
            // View column for reference field
            //
            $column = new TextViewColumn('reference', 'Reference', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Parameter_ReferencesGrid_reference_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            return $result;
        }
    ...
    try
    {
        $Page = new KBS_Parameter_ReferencesPage("KBS_Parameter_References.php", "KBS_Parameter_References", GetCurrentUserGrantForDataSource("KBS_Parameter_References"), 'UTF-8');
        $Page->SetShortCaption('KBS Parameter References');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('KBS Parameter References');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("KBS_Parameter_References"));
