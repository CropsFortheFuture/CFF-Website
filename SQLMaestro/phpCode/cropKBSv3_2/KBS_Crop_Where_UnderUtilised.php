 ...
 */
    ...
    class KBS_Crop_Where_UnderUtilisedPage extends Page
    {
        protected function DoBeforeCreate()
        {
            $this->dataset = new TableDataset(
                ...
                '`KBS_Crop_Where_UnderUtilised`');
            $field = new IntegerField('where_underUtilised_id');
            $this->dataset->AddField($field, false);
            $field = new IntegerField('cropID');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, true);
            $field = new IntegerField('utilised');
            $this->dataset->AddField($field, false);
            $field = new StringField('where_Underutilised');
            $this->dataset->AddField($field, false);
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('KBS_Crop_Where_UnderUtilisedssearch', $this->dataset,
                array('where_underUtilised_id', 'cropID_name_var_lndrce', 'utilised', 'where_Underutilised', 'info_id'),
                array($this->RenderText('Where UnderUtilised Id'), $this->RenderText('CropID'), $this->RenderText('Utilised'), $this->RenderText('Where Underutilised'), $this->RenderText('Info Id')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('KBS_Crop_Where_UnderUtilisedasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('where_underUtilised_id', $this->RenderText('Where UnderUtilised Id')));
            ...
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('utilised', $this->RenderText('Utilised')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('where_Underutilised', $this->RenderText('Where Underutilised')));
        ...
        {
            //
            // View column for where_underUtilised_id field
            //
            $column = new TextViewColumn('where_underUtilised_id', 'Where UnderUtilised Id', $this->dataset);
            ...
            //
            // View column for utilised field
            //
            $column = new TextViewColumn('utilised', 'Utilised', $this->dataset);
            $column->SetOrderable(true);
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
            ...
            //
            // View column for where_Underutilised field
            //
            $column = new TextViewColumn('where_Underutilised', 'Where Underutilised', $this->dataset);
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_Crop_Where_UnderUtilisedGrid_where_Underutilised_handler_list');
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
        ...
        protected function AddSingleRecordViewColumns(Grid $grid)
        {
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_Crop_Where_UnderUtilisedGrid_where_Underutilised_handler_view');
            $grid->AddSingleRecordViewColumn($column);
        ...
        {
            //
            // Edit column for where_underUtilised_id field
            //
            $editor = new TextEdit('where_underutilised_id_edit');
            $editColumn = new CustomEditColumn('Where UnderUtilised Id', 'where_underUtilised_id', $editor, $this->dataset);
            ...
            //
            // Edit column for utilised field
            //
            $editor = new TextEdit('utilised_edit');
            $editColumn = new CustomEditColumn('Utilised', 'utilised', $editor, $this->dataset);
            $editColumn->SetAllowSetToNull(true);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
            
            //
            // Edit column for where_Underutilised field
            //
            $editor = new TextEdit('where_underutilised_edit');
            $editor->SetSize(100);
            $editor->SetMaxLength(100);
            $editColumn = new CustomEditColumn('Where Underutilised', 'where_Underutilised', $editor, $this->dataset);
            $editColumn->SetAllowSetToNull(true);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
        ...
        public function GetModalGridDeleteHandler() { return 'KBS_Crop_Where_UnderUtilised_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'KBS_Crop_Where_UnderUtilisedGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
            ...
            $column = new TextViewColumn('where_Underutilised', 'Where Underutilised', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Crop_Where_UnderUtilisedGrid_where_Underutilised_handler_list', $column);
            GetApplication()->RegisterHTTPHandler($handler);//
            // View column for where_Underutilised field
            //
            $column = new TextViewColumn('where_Underutilised', 'Where Underutilised', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Crop_Where_UnderUtilisedGrid_where_Underutilised_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            return $result;
        }
    ...
    try
    {
        $Page = new KBS_Crop_Where_UnderUtilisedPage("KBS_Crop_Where_UnderUtilised.php", "KBS_Crop_Where_UnderUtilised", GetCurrentUserGrantForDataSource("KBS_Crop_Where_UnderUtilised"), 'UTF-8');
        $Page->SetShortCaption('KBS Crop Where UnderUtilised');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('KBS Crop Where UnderUtilised');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("KBS_Crop_Where_UnderUtilised"));
