 ...
 */
    ...
    class KBS_Parameter_AccuracyPage extends Page
    {
        protected function DoBeforeCreate()
        {
            $this->dataset = new TableDataset(
                ...
                '`KBS_Parameter_Accuracy`');
            $field = new IntegerField('acc_id');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, true);
            $field = new IntegerField('cropID');
            $this->dataset->AddField($field, false);
            $field = new StringField('data_author');
            $this->dataset->AddField($field, false);
            $field = new IntegerField('acc_exp');
            $this->dataset->AddField($field, false);
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('KBS_Parameter_Accuracyssearch', $this->dataset,
                array('acc_id', 'cropID_name_var_lndrce', 'data_author', 'acc_exp', 'info_id'),
                array($this->RenderText('Acc Id'), $this->RenderText('CropID'), $this->RenderText('Data Author'), $this->RenderText('Acc Exp'), $this->RenderText('Info Id')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('KBS_Parameter_Accuracyasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('acc_id', $this->RenderText('Acc Id')));
            ...
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('data_author', $this->RenderText('Data Author')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('acc_exp', $this->RenderText('Acc Exp')));
        ...
        {
            //
            // View column for acc_id field
            //
            $column = new TextViewColumn('acc_id', 'Acc Id', $this->dataset);
            ...
            //
            // View column for data_author field
            //
            $column = new TextViewColumn('data_author', 'Data Author', $this->dataset);
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_Parameter_AccuracyGrid_data_author_handler_list');
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
            
            //
            // View column for acc_exp field
            //
            $column = new TextViewColumn('acc_exp', 'Acc Exp', $this->dataset);
            $column->SetOrderable(true);
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
        ...
        protected function AddSingleRecordViewColumns(Grid $grid)
        {
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_Parameter_AccuracyGrid_data_author_handler_view');
            $grid->AddSingleRecordViewColumn($column);
        ...
        {
            //
            // Edit column for acc_id field
            //
            $editor = new TextEdit('acc_id_edit');
            $editColumn = new CustomEditColumn('Acc Id', 'acc_id', $editor, $this->dataset);
            ...
            //
            // Edit column for data_author field
            //
            $editor = new TextAreaEdit('data_author_edit', 50, 8);
            $editColumn = new CustomEditColumn('Data Author', 'data_author', $editor, $this->dataset);
            $editColumn->SetAllowSetToNull(true);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
            
            //
            // Edit column for acc_exp field
            //
            $editor = new TextEdit('acc_exp_edit');
            $editColumn = new CustomEditColumn('Acc Exp', 'acc_exp', $editor, $this->dataset);
            $editColumn->SetAllowSetToNull(true);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
        ...
        public function GetModalGridDeleteHandler() { return 'KBS_Parameter_Accuracy_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'KBS_Parameter_AccuracyGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
            ...
            $column = new TextViewColumn('data_author', 'Data Author', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Parameter_AccuracyGrid_data_author_handler_list', $column);
            GetApplication()->RegisterHTTPHandler($handler);//
            // View column for data_author field
            //
            $column = new TextViewColumn('data_author', 'Data Author', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Parameter_AccuracyGrid_data_author_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            return $result;
        }
    ...
    try
    {
        $Page = new KBS_Parameter_AccuracyPage("KBS_Parameter_Accuracy.php", "KBS_Parameter_Accuracy", GetCurrentUserGrantForDataSource("KBS_Parameter_Accuracy"), 'UTF-8');
        $Page->SetShortCaption('KBS Parameter Accuracy');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('KBS Parameter Accuracy');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("KBS_Parameter_Accuracy"));
