 ...
 */
    ...
    class Info_KBS_General_Parts_UsedPage extends Page
    {
        protected function DoBeforeCreate()
        {
            ...
            $this->dataset->AddField($field, false);
            $field = new IntegerField('part_used_petiold');
            $this->dataset->AddField($field, false);
            ...
            $this->dataset->AddField($field, false);
            $field = new IntegerField('kbs_part_used_others');
            $this->dataset->AddField($field, false);
        }
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('Info_KBS_General_Parts_Usedssearch', $this->dataset,
                array('cropID', 'usage_id', 'part_used_bark', 'part_used_bulb', 'part_used_flower', 'part_used_fruits', 'part_used_grain', 'part_used_leaf', 'part_used_petiold', 'part_used_root', 'part_used_seed', 'part_used_stem', 'part_used_tuber', 'part_used_whole', 'part_used_wood', 'kbs_part_used_others'),
                array($this->RenderText('CropID'), $this->RenderText('Usage Id'), $this->RenderText('Part Used Bark'), $this->RenderText('Part Used Bulb'), $this->RenderText('Part Used Flower'), $this->RenderText('Part Used Fruits'), $this->RenderText('Part Used Grain'), $this->RenderText('Part Used Leaf'), $this->RenderText('Part Used Petiold'), $this->RenderText('Part Used Root'), $this->RenderText('Part Used Seed'), $this->RenderText('Part Used Stem'), $this->RenderText('Part Used Tuber'), $this->RenderText('Part Used Whole'), $this->RenderText('Part Used Wood'), $this->RenderText('Kbs Part Used Others')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('Info_KBS_General_Parts_Usedasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            ...
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('part_used_petiold', $this->RenderText('Part Used Petiold')));
        ...
        protected function AddFieldColumns(Grid $grid)
        {
            ...
            //
            // View column for part_used_petiold field
            //
            $column = new TextViewColumn('part_used_petiold', 'Part Used Petiold', $this->dataset);
            $column->SetOrderable(true);
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            //
            // Edit column for part_used_petiold field
            //
            $editor = new TextEdit('part_used_petiold_edit');
            $editColumn = new CustomEditColumn('Part Used Petiold', 'part_used_petiold', $editor, $this->dataset);
            $editColumn->SetAllowSetToNull(true);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
            ...
            //
            $editor = new TextEdit('kbs_part_used_others_edit');
        ...
        public function GetModalGridDeleteHandler() { return 'Info_KBS_General_Parts_Used_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'Info_KBS_General_Parts_UsedGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
    ...
    try
    {
        $Page = new Info_KBS_General_Parts_UsedPage("Info_KBS_General_Parts_Used.php", "Info_KBS_General_Parts_Used", GetCurrentUserGrantForDataSource("Info_KBS_General_Parts_Used"), 'UTF-8');
        $Page->SetShortCaption('Info KBS General Parts Used');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('Info KBS General Parts Used');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("Info_KBS_General_Parts_Used"));
