 ...
 */
    ...
    class KBS_Crop_PestsPage extends Page
    {
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('KBS_Crop_Pestsssearch', $this->dataset,
                array('pest_id', 'pest_name', 'cropID_name_var_lndrce', 'info_id'),
                array($this->RenderText('Pest Id'), $this->RenderText('Pest Name'), $this->RenderText('CropID'), $this->RenderText('Info Id')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('KBS_Crop_Pestsasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
        ...
        protected function AddFieldColumns(Grid $grid)
        {
            ...
            $column->SetFullTextWindowHandlerName('KBS_Crop_PestsGrid_pest_name_handler_list');
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
        ...
        protected function AddSingleRecordViewColumns(Grid $grid)
        {
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_Crop_PestsGrid_pest_name_handler_view');
            $grid->AddSingleRecordViewColumn($column);
        ...
        public function GetModalGridDeleteHandler() { return 'KBS_Crop_Pests_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'KBS_Crop_PestsGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
            ...
            $column = new TextViewColumn('pest_name', 'Pest Name', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Crop_PestsGrid_pest_name_handler_list', $column);
            GetApplication()->RegisterHTTPHandler($handler);//
            // View column for pest_name field
            //
            $column = new TextViewColumn('pest_name', 'Pest Name', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Crop_PestsGrid_pest_name_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            return $result;
        }
    ...
    try
    {
        $Page = new KBS_Crop_PestsPage("KBS_Crop_Pests.php", "KBS_Crop_Pests", GetCurrentUserGrantForDataSource("KBS_Crop_Pests"), 'UTF-8');
        $Page->SetShortCaption('KBS Crop Pests');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('KBS Crop Pests');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("KBS_Crop_Pests"));
