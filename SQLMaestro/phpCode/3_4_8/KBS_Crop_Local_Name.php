 ...
 */
    ...
    class KBS_Crop_Local_NamePage extends Page
    {
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('KBS_Crop_Local_Namessearch', $this->dataset,
                array('localname_id', 'cropID_name_var_lndrce', 'localname', 'location_id_google_address', 'metadata_id'),
                array($this->RenderText('Localname Id'), $this->RenderText('CropID'), $this->RenderText('Localname'), $this->RenderText('Location Id'), $this->RenderText('Metadata Id')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('KBS_Crop_Local_Nameasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
        ...
        protected function AddFieldColumns(Grid $grid)
        {
            ...
            $column->SetFullTextWindowHandlerName('KBS_Crop_Local_NameGrid_localname_handler_list');
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
        ...
        protected function AddSingleRecordViewColumns(Grid $grid)
        {
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_Crop_Local_NameGrid_localname_handler_view');
            $grid->AddSingleRecordViewColumn($column);
        ...
        public function GetModalGridDeleteHandler() { return 'KBS_Crop_Local_Name_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'KBS_Crop_Local_NameGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
            ...
            $column = new TextViewColumn('localname', 'Localname', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Crop_Local_NameGrid_localname_handler_list', $column);
            GetApplication()->RegisterHTTPHandler($handler);//
            // View column for localname field
            //
            $column = new TextViewColumn('localname', 'Localname', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Crop_Local_NameGrid_localname_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            return $result;
        }
    ...
    try
    {
        $Page = new KBS_Crop_Local_NamePage("KBS_Crop_Local_Name.php", "KBS_Crop_Local_Name", GetCurrentUserGrantForDataSource("KBS_Crop_Local_Name"), 'UTF-8');
        $Page->SetShortCaption('KBS Crop Local Name');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('KBS Crop Local Name');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("KBS_Crop_Local_Name"));
