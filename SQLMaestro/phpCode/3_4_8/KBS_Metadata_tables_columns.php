 ...
 */
    ...
    class KBS_Metadata_tables_columnsPage extends Page
    {
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('KBS_Metadata_tables_columnsssearch', $this->dataset,
                array('id', 'table_name', 'col_name', 'table_col_name'),
                array($this->RenderText('Id'), $this->RenderText('Table Name'), $this->RenderText('Col Name'), $this->RenderText('Table Col Name')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('KBS_Metadata_tables_columnsasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            ...
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('table_col_name', $this->RenderText('Table Col Name')));
        }
        ...
        protected function AddFieldColumns(Grid $grid)
        {
            ...
            //
            $column = new TextViewColumn('table_col_name', 'Table Col Name', $this->dataset);
            $column->SetOrderable(true);
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            //
            $editor = new TextEdit('col_name_edit');
            $editor->SetSize(45);
            $editor->SetMaxLength(45);
            ...
            //
            // Edit column for table_col_name field
            //
            $editor = new TextEdit('table_col_name_edit');
            $editor->SetSize(45);
            $editor->SetMaxLength(45);
            $editColumn = new CustomEditColumn('Table Col Name', 'table_col_name', $editor, $this->dataset);
            $editColumn->SetAllowSetToNull(true);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
        }
        ...
        public function GetModalGridDeleteHandler() { return 'KBS_Metadata_tables_columns_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'KBS_Metadata_tables_columnsGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
    ...
    try
    {
        $Page = new KBS_Metadata_tables_columnsPage("KBS_Metadata_tables_columns.php", "KBS_Metadata_tables_columns", GetCurrentUserGrantForDataSource("KBS_Metadata_tables_columns"), 'UTF-8');
        $Page->SetShortCaption('KBS Metadata Tables Columns');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('KBS Metadata Tables Columns');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("KBS_Metadata_tables_columns"));
