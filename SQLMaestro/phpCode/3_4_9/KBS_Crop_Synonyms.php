 ...
 */
    ...
    class KBS_Crop_SynonymsPage extends Page
    {
        protected function DoBeforeCreate()
        {
            $this->dataset = new TableDataset(
                ...
                '`KBS_Crop_Synonyms`');
            $field = new IntegerField('synonym_id', null, null, true);
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, true);
            $field = new StringField('sci_name_syn');
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('KBS_Crop_Synonymsssearch', $this->dataset,
                array('synonym_id', 'sci_name_syn', 'cropID_name_var_lndrce', 'metadata_id'),
                array($this->RenderText('Synonym Id'), $this->RenderText('Sci Name Syn'), $this->RenderText('CropID'), $this->RenderText('Metadata Id')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('KBS_Crop_Synonymsasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('synonym_id', $this->RenderText('Synonym Id')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('sci_name_syn', $this->RenderText('Sci Name Syn')));
        ...
        {
            //
            // View column for synonym_id field
            //
            $column = new TextViewColumn('synonym_id', 'Synonym Id', $this->dataset);
            ...
            //
            // View column for sci_name_syn field
            //
            $column = new TextViewColumn('sci_name_syn', 'Sci Name Syn', $this->dataset);
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_Crop_SynonymsGrid_sci_name_syn_handler_list');
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
        ...
        protected function AddSingleRecordViewColumns(Grid $grid)
        {
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_Crop_SynonymsGrid_sci_name_syn_handler_view');
            $grid->AddSingleRecordViewColumn($column);
        ...
        {
            //
            // Edit column for sci_name_syn field
            //
            $editor = new TextAreaEdit('sci_name_syn_edit', 50, 8);
            $editColumn = new CustomEditColumn('Sci Name Syn', 'sci_name_syn', $editor, $this->dataset);
        ...
        public function GetModalGridDeleteHandler() { return 'KBS_Crop_Synonyms_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'KBS_Crop_SynonymsGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
            ...
            $column = new TextViewColumn('sci_name_syn', 'Sci Name Syn', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Crop_SynonymsGrid_sci_name_syn_handler_list', $column);
            GetApplication()->RegisterHTTPHandler($handler);//
            // View column for sci_name_syn field
            //
            $column = new TextViewColumn('sci_name_syn', 'Sci Name Syn', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Crop_SynonymsGrid_sci_name_syn_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            return $result;
        }
    ...
    try
    {
        $Page = new KBS_Crop_SynonymsPage("KBS_Crop_Synonyms.php", "KBS_Crop_Synonyms", GetCurrentUserGrantForDataSource("KBS_Crop_Synonyms"), 'UTF-8');
        $Page->SetShortCaption('KBS Crop Synonyms');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('KBS Crop Synonyms');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("KBS_Crop_Synonyms"));
