 ...
 */
    ...
    class KBS_SocioEconomy_Water_SourcePage extends Page
    {
        protected function DoBeforeCreate()
        {
            ...
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, true);
            $field = new IntegerField('water_source_borewell');
            $this->dataset->AddField($field, false);
            $field = new IntegerField('water_source_artesian');
            $this->dataset->AddField($field, false);
            $field = new IntegerField('water_source_river');
            $this->dataset->AddField($field, false);
            $field = new IntegerField('water_source_rainfed');
            $this->dataset->AddField($field, false);
            $field = new StringField('kbs_watersource_others');
            $this->dataset->AddField($field, false);
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('KBS_SocioEconomy_Water_Sourcessearch', $this->dataset,
                array('cropID_name_var_lndrce', 'location_ID_google_address', 'water_source_borewell', 'water_source_artesian', 'water_source_river', 'water_source_rainfed', 'kbs_watersource_others'),
                array($this->RenderText('CropID'), $this->RenderText('Location ID'), $this->RenderText('Water Source Borewell'), $this->RenderText('Water Source Artesian'), $this->RenderText('Water Source River'), $this->RenderText('Water Source Rainfed'), $this->RenderText('Kbs Watersource Others')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('KBS_SocioEconomy_Water_Sourceasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            ...
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('water_source_borewell', $this->RenderText('Water Source Borewell')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('water_source_artesian', $this->RenderText('Water Source Artesian')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('water_source_river', $this->RenderText('Water Source River')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('water_source_rainfed', $this->RenderText('Water Source Rainfed')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('kbs_watersource_others', $this->RenderText('Kbs Watersource Others')));
        }
        ...
        protected function AddFieldColumns(Grid $grid)
        {
            ...
            //
            // View column for water_source_borewell field
            //
            $column = new TextViewColumn('water_source_borewell', 'Water Source Borewell', $this->dataset);
            $column->SetOrderable(true);
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
            ...
            //
            // View column for water_source_artesian field
            //
            $column = new TextViewColumn('water_source_artesian', 'Water Source Artesian', $this->dataset);
            $column->SetOrderable(true);
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
            ...
            //
            // View column for water_source_river field
            //
            $column = new TextViewColumn('water_source_river', 'Water Source River', $this->dataset);
            $column->SetOrderable(true);
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
            ...
            //
            // View column for water_source_rainfed field
            //
            $column = new TextViewColumn('water_source_rainfed', 'Water Source Rainfed', $this->dataset);
            $column->SetOrderable(true);
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
            ...
            //
            // View column for kbs_watersource_others field
            //
            $column = new TextViewColumn('kbs_watersource_others', 'Kbs Watersource Others', $this->dataset);
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_SocioEconomy_Water_SourceGrid_kbs_watersource_others_handler_list');
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
        }
        ...
        protected function AddSingleRecordViewColumns(Grid $grid)
        {
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_SocioEconomy_Water_SourceGrid_kbs_watersource_others_handler_view');
            $grid->AddSingleRecordViewColumn($column);
        }
        ...
        protected function AddEditColumns(Grid $grid)
        {
            ...
            //
            // Edit column for water_source_borewell field
            //
            $editor = new TextEdit('water_source_borewell_edit');
            $editColumn = new CustomEditColumn('Water Source Borewell', 'water_source_borewell', $editor, $this->dataset);
            $editColumn->SetAllowSetToNull(true);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
            
            //
            // Edit column for water_source_artesian field
            //
            $editor = new TextEdit('water_source_artesian_edit');
            $editColumn = new CustomEditColumn('Water Source Artesian', 'water_source_artesian', $editor, $this->dataset);
            $editColumn->SetAllowSetToNull(true);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
            
            //
            // Edit column for water_source_river field
            //
            $editor = new TextEdit('water_source_river_edit');
            $editColumn = new CustomEditColumn('Water Source River', 'water_source_river', $editor, $this->dataset);
            $editColumn->SetAllowSetToNull(true);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
            
            //
            // Edit column for water_source_rainfed field
            //
            $editor = new TextEdit('water_source_rainfed_edit');
            $editColumn = new CustomEditColumn('Water Source Rainfed', 'water_source_rainfed', $editor, $this->dataset);
            $editColumn->SetAllowSetToNull(true);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
            
            //
            // Edit column for kbs_watersource_others field
            //
            $editor = new TextAreaEdit('kbs_watersource_others_edit', 50, 8);
            $editColumn = new CustomEditColumn('Kbs Watersource Others', 'kbs_watersource_others', $editor, $this->dataset);
            $editColumn->SetAllowSetToNull(true);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
        }
        ...
        public function GetModalGridDeleteHandler() { return 'KBS_SocioEconomy_Water_Source_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'KBS_SocioEconomy_Water_SourceGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
            ...
            $column = new TextViewColumn('kbs_watersource_others', 'Kbs Watersource Others', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_SocioEconomy_Water_SourceGrid_kbs_watersource_others_handler_list', $column);
            GetApplication()->RegisterHTTPHandler($handler);//
            // View column for kbs_watersource_others field
            //
            $column = new TextViewColumn('kbs_watersource_others', 'Kbs Watersource Others', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_SocioEconomy_Water_SourceGrid_kbs_watersource_others_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            return $result;
        }
    ...
    try
    {
        $Page = new KBS_SocioEconomy_Water_SourcePage("KBS_SocioEconomy_Water_Source.php", "KBS_SocioEconomy_Water_Source", GetCurrentUserGrantForDataSource("KBS_SocioEconomy_Water_Source"), 'UTF-8');
        $Page->SetShortCaption('KBS SocioEconomy Water Source');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('KBS SocioEconomy Water Source');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("KBS_SocioEconomy_Water_Source"));
