 ...
 */
    ...
    class KBS_Global_Country_LatLongPage extends Page
    {
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('KBS_Global_Country_LatLongssearch', $this->dataset,
                array('id', 'latitude', 'longitude', 'name'),
                array($this->RenderText('Id'), $this->RenderText('Latitude'), $this->RenderText('Longitude'), $this->RenderText('Name')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('KBS_Global_Country_LatLongasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
        ...
        protected function AddFieldColumns(Grid $grid)
        {
            ...
            $column->SetFullTextWindowHandlerName('KBS_Global_Country_LatLongGrid_name_handler_list');
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
        }
        ...
        protected function AddSingleRecordViewColumns(Grid $grid)
        {
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_Global_Country_LatLongGrid_name_handler_view');
            $grid->AddSingleRecordViewColumn($column);
        }
        ...
        public function GetModalGridDeleteHandler() { return 'KBS_Global_Country_LatLong_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'KBS_Global_Country_LatLongGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
            ...
            $column = new TextViewColumn('name', 'Name', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Global_Country_LatLongGrid_name_handler_list', $column);
            GetApplication()->RegisterHTTPHandler($handler);//
            // View column for name field
            //
            $column = new TextViewColumn('name', 'Name', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Global_Country_LatLongGrid_name_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            return $result;
        }
    ...
    try
    {
        $Page = new KBS_Global_Country_LatLongPage("KBS_Global_Country_LatLong.php", "KBS_Global_Country_LatLong", GetCurrentUserGrantForDataSource("KBS_Global_Country_LatLong"), 'UTF-8');
        $Page->SetShortCaption('KBS Global Country LatLong');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('KBS Global Country LatLong');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("KBS_Global_Country_LatLong"));
