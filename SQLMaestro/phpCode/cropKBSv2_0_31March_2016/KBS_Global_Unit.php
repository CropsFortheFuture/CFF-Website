 ...
 */
    ...
    class KBS_Global_UnitPage extends Page
    {
        protected function DoBeforeCreate()
        {
            $this->dataset = new TableDataset(
                ...
                '`KBS_Global_Unit`');
            $field = new IntegerField('unitID');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, true);
            $field = new StringField('unitValue');
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('KBS_Global_Unitssearch', $this->dataset,
                array('unitID', 'unitValue'),
                array($this->RenderText('UnitID'), $this->RenderText('UnitValue')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('KBS_Global_Unitasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('unitID', $this->RenderText('UnitID')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('unitValue', $this->RenderText('UnitValue')));
        }
        ...
        {
            //
            // View column for unitID field
            //
            $column = new TextViewColumn('unitID', 'UnitID', $this->dataset);
            ...
            //
            // View column for unitValue field
            //
            $column = new TextViewColumn('unitValue', 'UnitValue', $this->dataset);
            $column->SetOrderable(true);
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
        ...
        {
            //
            // Edit column for unitID field
            //
            $editor = new TextEdit('unitid_edit');
            $editColumn = new CustomEditColumn('UnitID', 'unitID', $editor, $this->dataset);
            ...
            //
            // Edit column for unitValue field
            //
            $editor = new TextEdit('unitvalue_edit');
            $editor->SetSize(45);
            $editor->SetMaxLength(45);
            $editColumn = new CustomEditColumn('UnitValue', 'unitValue', $editor, $this->dataset);
        ...
        public function GetModalGridDeleteHandler() { return 'KBS_Global_Unit_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'KBS_Global_UnitGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
    ...
    try
    {
        $Page = new KBS_Global_UnitPage("KBS_Global_Unit.php", "KBS_Global_Unit", GetCurrentUserGrantForDataSource("KBS_Global_Unit"), 'UTF-8');
        $Page->SetShortCaption('KBS Global Unit');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('KBS Global Unit');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("KBS_Global_Unit"));
