 ...
 */
    ...
    class KBS_Global_WeedsPage extends Page
    {
        protected function DoBeforeCreate()
        {
            $this->dataset = new TableDataset(
                ...
                '`KBS_Global_Weeds`');
            $field = new IntegerField('weed_id', null, null, true);
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, true);
            $field = new StringField('weed_sci_name');
            $this->dataset->AddField($field, false);
            $field = new StringField('weed_name');
            $this->dataset->AddField($field, false);
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('KBS_Global_Weedsssearch', $this->dataset,
                array('weed_id', 'weed_sci_name', 'weed_name', 'metadata_id'),
                array($this->RenderText('Weed Id'), $this->RenderText('Weed Sci Name'), $this->RenderText('Weed Name'), $this->RenderText('Metadata Id')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('KBS_Global_Weedsasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('weed_id', $this->RenderText('Weed Id')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('weed_sci_name', $this->RenderText('Weed Sci Name')));
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('weed_name', $this->RenderText('Weed Name')));
        ...
        {
            //
            // View column for weed_id field
            //
            $column = new TextViewColumn('weed_id', 'Weed Id', $this->dataset);
            ...
            //
            // View column for weed_sci_name field
            //
            $column = new TextViewColumn('weed_sci_name', 'Weed Sci Name', $this->dataset);
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_Global_WeedsGrid_weed_sci_name_handler_list');
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
            
            //
            // View column for weed_name field
            //
            $column = new TextViewColumn('weed_name', 'Weed Name', $this->dataset);
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_Global_WeedsGrid_weed_name_handler_list');
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
        ...
        protected function AddSingleRecordViewColumns(Grid $grid)
        {
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_Global_WeedsGrid_weed_sci_name_handler_view');
            $grid->AddSingleRecordViewColumn($column);
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_Global_WeedsGrid_weed_name_handler_view');
            $grid->AddSingleRecordViewColumn($column);
        ...
        {
            //
            // Edit column for weed_id field
            //
            $editor = new TextEdit('weed_id_edit');
            $editColumn = new CustomEditColumn('Weed Id', 'weed_id', $editor, $this->dataset);
            ...
            //
            // Edit column for weed_sci_name field
            //
            $editor = new TextEdit('weed_sci_name_edit');
            $editor->SetSize(100);
            $editor->SetMaxLength(100);
            $editColumn = new CustomEditColumn('Weed Sci Name', 'weed_sci_name', $editor, $this->dataset);
            $editColumn->SetAllowSetToNull(true);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
            
            //
            // Edit column for weed_name field
            //
            $editor = new TextEdit('weed_name_edit');
            $editor->SetSize(100);
            $editor->SetMaxLength(100);
            $editColumn = new CustomEditColumn('Weed Name', 'weed_name', $editor, $this->dataset);
            $editColumn->SetAllowSetToNull(true);
            $this->ApplyCommonColumnEditProperties($editColumn);
            $grid->AddEditColumn($editColumn);
        ...
        public function GetModalGridDeleteHandler() { return 'KBS_Global_Weeds_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'KBS_Global_WeedsGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
            ...
            $column = new TextViewColumn('weed_sci_name', 'Weed Sci Name', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Global_WeedsGrid_weed_sci_name_handler_list', $column);
            ...
            $column = new TextViewColumn('weed_name', 'Weed Name', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Global_WeedsGrid_weed_name_handler_list', $column);
            GetApplication()->RegisterHTTPHandler($handler);//
            // View column for weed_sci_name field
            //
            $column = new TextViewColumn('weed_sci_name', 'Weed Sci Name', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Global_WeedsGrid_weed_sci_name_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            //
            // View column for weed_name field
            //
            $column = new TextViewColumn('weed_name', 'Weed Name', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Global_WeedsGrid_weed_name_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            return $result;
        }
    ...
    try
    {
        $Page = new KBS_Global_WeedsPage("KBS_Global_Weeds.php", "KBS_Global_Weeds", GetCurrentUserGrantForDataSource("KBS_Global_Weeds"), 'UTF-8');
        $Page->SetShortCaption('KBS Global Weeds');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('KBS Global Weeds');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("KBS_Global_Weeds"));
