 ...
 */
    ...
    class KBS_Crop_CRO_FilePage extends Page
    {
        protected function DoBeforeCreate()
        {
            $this->dataset = new TableDataset(
                ...
                '`KBS_Crop_CRO_File`');
            $field = new IntegerField('CRO_id', null, null, true);
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, true);
            $field = new IntegerField('cropID');
            $field->SetIsNotNull(true);
            $this->dataset->AddField($field, false);
            $field = new StringField('CRO_path');
            $this->dataset->AddField($field, false);
        ...
        {
            $grid->UseFilter = true;
            $grid->SearchControl = new SimpleSearch('KBS_Crop_CRO_Filessearch', $this->dataset,
                array('CRO_id', 'cropID_name_var_lndrce', 'CRO_path', 'metadata_id'),
                array($this->RenderText('CRO Id'), $this->RenderText('CropID'), $this->RenderText('CRO Path'), $this->RenderText('Metadata Id')),
                array(
        ...
        {
            $this->AdvancedSearchControl = new AdvancedSearchControl('KBS_Crop_CRO_Fileasearch', $this->dataset, $this->GetLocalizerCaptions(), $this->GetColumnVariableContainer(), $this->CreateLinkBuilder());
            $this->AdvancedSearchControl->setTimerInterval(1000);
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('CRO_id', $this->RenderText('CRO Id')));
            ...
            $this->AdvancedSearchControl->AddSearchColumn($this->AdvancedSearchControl->CreateStringSearchInput('CRO_path', $this->RenderText('CRO Path')));
        ...
        {
            //
            // View column for CRO_id field
            //
            $column = new TextViewColumn('CRO_id', 'CRO Id', $this->dataset);
            ...
            //
            // View column for CRO_path field
            //
            $column = new TextViewColumn('CRO_path', 'CRO Path', $this->dataset);
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_Crop_CRO_FileGrid_CRO_path_handler_list');
            $column->SetDescription($this->RenderText(''));
            $column->SetFixedWidth(null);
            $grid->AddViewColumn($column);
        ...
        protected function AddSingleRecordViewColumns(Grid $grid)
        {
            ...
            $column->SetOrderable(true);
            $column->SetMaxLength(75);
            $column->SetFullTextWindowHandlerName('KBS_Crop_CRO_FileGrid_CRO_path_handler_view');
            $grid->AddSingleRecordViewColumn($column);
        ...
        {
            //
            // Edit column for CRO_id field
            //
            $editor = new TextEdit('cro_id_edit');
            $editColumn = new CustomEditColumn('CRO Id', 'CRO_id', $editor, $this->dataset);
            ...
            //
            // Edit column for CRO_path field
            //
            $editor = new TextEdit('cro_path_edit');
            $editor->SetSize(100);
            $editor->SetMaxLength(100);
            $editColumn = new CustomEditColumn('CRO Path', 'CRO_path', $editor, $this->dataset);
        ...
        public function GetModalGridDeleteHandler() { return 'KBS_Crop_CRO_File_modal_delete'; }
        protected function GetEnableModalGridDelete() { return true; }
    
        protected function CreateGrid()
        {
            $result = new Grid($this, $this->dataset, 'KBS_Crop_CRO_FileGrid');
            if ($this->GetSecurityInfo()->HasDeleteGrant())
            ...
            $column = new TextViewColumn('CRO_path', 'CRO Path', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Crop_CRO_FileGrid_CRO_path_handler_list', $column);
            GetApplication()->RegisterHTTPHandler($handler);//
            // View column for CRO_path field
            //
            $column = new TextViewColumn('CRO_path', 'CRO Path', $this->dataset);
            $column->SetOrderable(true);
            $handler = new ShowTextBlobHandler($this->dataset, $this, 'KBS_Crop_CRO_FileGrid_CRO_path_handler_view', $column);
            GetApplication()->RegisterHTTPHandler($handler);
            return $result;
        }
    ...
    try
    {
        $Page = new KBS_Crop_CRO_FilePage("KBS_Crop_CRO_File.php", "KBS_Crop_CRO_File", GetCurrentUserGrantForDataSource("KBS_Crop_CRO_File"), 'UTF-8');
        $Page->SetShortCaption('KBS Crop CRO File');
        $Page->SetHeader(GetPagesHeader());
        $Page->SetFooter(GetPagesFooter());
        $Page->SetCaption('KBS Crop CRO File');
        $Page->SetRecordPermission(GetCurrentUserRecordPermissionsForDataSource("KBS_Crop_CRO_File"));
